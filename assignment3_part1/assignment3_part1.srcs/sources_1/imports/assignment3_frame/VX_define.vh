...
/** please update the following define with your own values */ 
  ...
  `define DE_latch_WIDTH  (`INSTBITS+`DBITS+`DBITS+`OP1BITS+`OP2BITS+`DBITS+`DBITS+`DBITS + 1 + 1 + 1 + 1 + 1+ `REGNOBITS + 8 + `BUS_CANARY_WIDTH)
  ...
  `define from_AGEX_to_DE_WIDTH (1 + 1 + 1 + `DBITS + `DBITS ) // is_br, is_jmp, br_cond, br_target, jmp_target
  `define from_MEM_to_DE_WIDTH 4 
  `define from_WB_to_DE_WIDTH  (1 + 1 + `REGNOBITS + `DBITS + `DBITS)

  `define from_MEM_to_AGEX_WIDTH  4 + 1 + `DBITS
  `define from_WB_to_AGEX_WIDTH  4 + 1 + `DBITS
